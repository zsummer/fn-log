...
int main(int argc, char* argv[])
{
    ...
    do
    {
        long long last_writed = AtomicLoadL(logger.shm_->channels_[1], FNLog::CHANNEL_LOG_PROCESSED);
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        long long now_writed = AtomicLoadL(logger.shm_->channels_[1], FNLog::CHANNEL_LOG_PROCESSED);
