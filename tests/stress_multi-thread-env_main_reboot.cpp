...
int main(int argc, char* argv[])
{
    ...
    {
        long long last_writed = AtomicLoadChannelLog(logger.shm_->channels_[0], FNLog::CHANNEL_LOG_PROCESSED);
        std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        long long now_writed = AtomicLoadChannelLog(logger.shm_->channels_[0], FNLog::CHANNEL_LOG_PROCESSED);
